        ...
        <div className="card">
          {sequences.length > 0 ? (
            <div style={{ display: 'flex', flexDirection: 'column', gap: 4 }}>
              {sequences.map((s, i) => (
                <div key={s.id} style={{ display: 'flex', gap: 10, alignItems: 'center', padding: '10px 12px', background: 'rgba(92,184,178,0.03)', borderRadius: 8, border: '1px solid var(--border-subtle)' }}>
                  <div style={{ width: 28, height: 28, borderRadius: '50%', background: '#5CB8B220', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
                    <span style={{ fontSize: 12, fontWeight: 700, color: '#5CB8B2' }}>{i + 1}</span>
                  </div>
                  {editingId === s.id ? (
            ...
            <div className="empty-state"><ClipboardList size={32} style={{ color: '#A0AEC0', marginBottom: 8 }} /><h3>Nenhuma pergunta de qualificacao</h3><p>Defina perguntas para qualificar seus leads no detalhe do lead.</p></div>
          )}
        </div>
      )}
