...
const DEFAULT_COLORS = ['#5CB8B2', '#5DADE2', '#9B59B6', '#8DD4CF', '#FF6B8A', '#34C759', '#FF6B6B']
            ...
            <div key={f.id} className="card">
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 12 }}>
                <div>
                  ...
                  {f.is_default ? <span style={{ fontSize: 10, color: '#5CB8B2' }}>PADRAO</span> : null}
                </div>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 8, marginTop: 12 }}>
              {editStages.map((s, i) => (
                <div key={i} style={{ display: 'flex', gap: 8, alignItems: 'center' }}>
                  <GripVertical size={14} style={{ color: '#A0AEC0', cursor: 'grab' }} />
                  <input type="color" value={s.color || '#5CB8B2'} onChange={e => updateStage(i, 'color', e.target.value)} style={{ width: 30, height: 30, border: 'none', background: 'none', cursor: 'pointer' }} />
      ...
      {showNew && (
        <div className="modal-overlay" onClick={() => setShowNew(false)}>
          <div className="modal" onClick={e => e.stopPropagation()}>
            ...
            <p style={{ fontSize: 11, color: '#718096', marginTop: 8 }}>Um funil padrao sera criado com etapas: Novo Lead, Em Atendimento, Qualificado, Venda, Perdido. Voce pode editar depois.</p>
            <div className="modal-actions">
