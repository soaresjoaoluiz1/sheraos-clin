  ...
  return (
    <div>
      ...
      <section className="dash-section">
        ...
        <div className="card">
          <p style={{ fontSize: 12, color: '#718096', marginBottom: 12 }}>Configure uma vez a URL e API Key do seu servidor Evolution API. Todos os numeros WhatsApp desta conta usarao estas credenciais.</p>
          <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
            <div style={{ flex: 1, minWidth: 200 }}>
              <label style={{ fontSize: 11, color: '#718096', display: 'block', marginBottom: 4 }}>URL da API</label>
              <input className="input" value={evoUrl} onChange={e => setEvoUrl(e.target.value)} placeholder="https://evo.exemplo.com.br" />
            </div>
            <div style={{ flex: 1, minWidth: 200 }}>
              <label style={{ fontSize: 11, color: '#718096', display: 'block', marginBottom: 4 }}>API Key</label>
      ...
      {qrInstance && qrInstance.qr_code && qrInstance.status === 'connecting' && (
        <div className="card" style={{ marginBottom: 20, textAlign: 'center', padding: 24 }}>
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 8, marginBottom: 16 }}>
            <QrCode size={20} style={{ color: '#5CB8B2' }} />
          ...
          <div style={{ fontSize: 13, color: '#718096', maxWidth: 400, margin: '0 auto', lineHeight: 1.6 }}>
                ...
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: 12 }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
                    ...
                    <div>
                      ...
                      {inst.phone_number && <div style={{ fontSize: 12, color: '#4A5568' }}>{inst.phone_number}</div>}
                    </div>
                  </div>
      ...
      {!evoConfigured && (
        <div className="card" style={{ textAlign: 'center', padding: 30 }}>
          <Smartphone size={32} style={{ color: '#A0AEC0', marginBottom: 8 }} />
          <h3>Configure a Evolution API acima</h3>
          <p style={{ color: '#718096', fontSize: 13 }}>Salve a URL e API Key do servidor Evolution para comecar a conectar numeros de WhatsApp.</p>
        </div>
      )}
      ...
      {showNew && (
        <div className="modal-overlay" onClick={() => setShowNew(false)}>
          <div className="modal" onClick={e => e.stopPropagation()}>
            ...
            <p style={{ fontSize: 12, color: '#718096', marginBottom: 16 }}>De um nome para identificar este numero (ex: Comercial, Suporte, Vendas).</p>
