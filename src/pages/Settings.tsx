  ...
  return (
    <div>
      ...
      <section className="dash-section">
        ...
        <p style={{ fontSize: 12, color: '#718096', marginBottom: 16 }}>Configure como os leads sao distribuidos automaticamente quando chegam via WhatsApp, formularios ou site.</p>
            ...
            <div key={funnel.id} className="card" style={{ marginBottom: 12 }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 12 }}>
                <div>
                  ...
                  {funnel.is_default ? <span style={{ fontSize: 10, color: '#5CB8B2' }}>FUNIL PADRAO</span> : null}
                </div>
              ...
              {rule.type === 'manual' && (
                <div style={{ fontSize: 12, color: '#718096', padding: '8px 12px', background: 'rgba(255,255,255,0.03)', borderRadius: 6 }}>
                    ...
                    <div style={{ display: 'flex', flexDirection: 'column', gap: 4 }}>
                      {attendants.map(att => (
                        <label key={att.id} style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '6px 8px', borderRadius: 6, cursor: 'pointer',
                          ...
                          <span style={{ fontSize: 11, color: '#718096' }}>{att.email}</span>
                        </label>
                      ))}
                    </div>
                  )}
