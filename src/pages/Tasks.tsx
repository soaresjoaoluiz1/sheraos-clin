    ...
    return (
      <div key={taskKey} className="card" style={{ padding: 14, marginBottom: 10, borderLeft: isStandalone ? '3px solid #9B59B6' : undefined }}>
        <div style={{ display: 'flex', gap: 12, alignItems: 'flex-start' }}>
          {/* Avatar */}
          <div style={{ width: 40, height: 40, borderRadius: '50%', background: isStandalone ? '#9B59B625' : (t.stage_color ? `${t.stage_color}25` : '#5CB8B225'), display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0, overflow: 'hidden' }}>
            {t.profile_pic_url ? (
              ...
              <User size={18} style={{ color: isStandalone ? '#9B59B6' : (t.stage_color || '#5CB8B2') }} />
            )}
          </div>
          ...
          <div style={{ flex: 1, minWidth: 0 }}>
            ...
            {isStandalone ? (
              <>
                {t.lead_name && <div style={{ fontSize: 12, color: '#4A5568', marginBottom: 4, display: 'flex', alignItems: 'center', gap: 4 }}><User size={11} /> {t.lead_name}</div>}
                {t.description && <div style={{ fontSize: 11, color: '#718096', marginBottom: 4 }}>{t.description}</div>}
        ...
        <div className="modal-overlay" onClick={() => setConfirmModal(null)}>
          <div className="modal" style={{ maxWidth: 420 }} onClick={e => e.stopPropagation()}>
            <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 12, textAlign: 'center', padding: '8px 0' }}>
              ...
              <div style={{ fontSize: 13, color: '#718096' }}>Para <b style={{ color: '#fff' }}>{confirmModal.leadName}</b></div>
