...
const COLORS = ['#5CB8B2', '#34C759', '#5DADE2', '#FF6B8A', '#9B59B6', '#8DD4CF', '#EA4335', '#2ECC71']
...
function Tip({ active, payload, label }: any) {
  ...
  return (
    <div style={{ background: '#130A24', border: '1px solid rgba(255,255,255,0.15)', borderRadius: 8, padding: '10px 14px', fontSize: 12 }}>
      <p style={{ color: '#718096', marginBottom: 6 }}>{label}</p>
    ...
    <div>
      ...
      <section className="dash-section">
        <div className="metrics-grid">
          <div className="metric-card"><div className="metric-header"><span className="metric-label">Total Leads</span><div className="metric-icon" style={{ background: '#5CB8B220', color: '#5CB8B2' }}><Users size={16} /></div></div>
            ...
            <div className="chart-card">
              ...
              <ResponsiveContainer width="100%" height={220}>
                <AreaChart data={dailyData}>
                  <defs><linearGradient id="leadG" x1="0" y1="0" x2="0" y2="1"><stop offset="0%" stopColor="#5CB8B2" stopOpacity={0.3} /><stop offset="100%" stopColor="#5CB8B2" stopOpacity={0} /></linearGradient></defs>
                  <CartesianGrid strokeDasharray="3 3" stroke="rgba(255,255,255,0.06)" />
                  <XAxis dataKey="day" tick={{ fill: '#718096', fontSize: 10 }} /><YAxis tick={{ fill: '#718096', fontSize: 10 }} allowDecimals={false} />
                  <Tooltip content={<Tip />} /><Area type="monotone" dataKey="Leads" stroke="#5CB8B2" fill="url(#leadG)" strokeWidth={2} />
                </AreaChart>
              </ResponsiveContainer>
            </div>
          )}
          ...
          <div className="table-card">
            <table>
              ...
              <tbody>
                {agents.map(a => (
                  <tr key={a.id}>
                    <td className="name" style={{ display: 'flex', alignItems: 'center', gap: 6 }}><span style={{ width: 6, height: 6, borderRadius: '50%', background: a.is_active ? '#34C759' : '#718096' }} />{a.name}</td>
                    <td className="right" style={{ fontWeight: 600, color: '#2D3748' }}>{a.leads_period}</td>
