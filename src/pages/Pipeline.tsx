            ...
            <div key={stage.id} className="kanban-mobile-stage">
              <div className="kanban-mobile-stage-header" onClick={() => toggleStage(stage.id)}>
                <div className="kanban-mobile-stage-title">
                  ...
                  {stage.name}
                  <span style={{ background: 'rgba(255,255,255,0.08)', padding: '2px 8px', borderRadius: 10, fontSize: 12, color: '#718096' }}>{stageLeads.length}</span>
                  {metric?.conversion_from_prev != null && <span style={{ fontSize: 10, color: '#718096' }}>{metric.conversion_from_prev.toFixed(0)}%</span>}
                </div>
                {expanded ? <ChevronDown size={16} style={{ color: '#718096' }} /> : <ChevronRight size={16} style={{ color: '#718096' }} />}
              </div>
                    ...
                    <div key={lead.id} className="kanban-mobile-card">
                      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
                        <div onClick={() => navigate(`/leads/${lead.id}`)} style={{ cursor: 'pointer', flex: 1 }}>
                          ...
                          {lead.phone && <div style={{ fontSize: 12, color: '#718096', display: 'flex', alignItems: 'center', gap: 4, marginTop: 2 }}><Phone size={10} /> {lead.phone}</div>}
                        </div>
                      ...
                      <div style={{ display: 'flex', gap: 8, marginTop: 6, fontSize: 10, color: '#A0AEC0', flexWrap: 'wrap' }}>
              ...
              {expanded && stageLeads.length === 0 && (
                <div style={{ padding: '20px 14px', textAlign: 'center', color: '#A0AEC0', fontSize: 12 }}>Nenhum lead nesta etapa</div>
              )}
            </div>
          )
        })}
              ...
              <div className="kanban-column-header">
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
                  {metric?.conversion_from_prev != null && (
                    <span style={{ fontSize: 9, color: '#718096', fontWeight: 500 }}>{metric.conversion_from_prev.toFixed(0)}%</span>
                  )}
                  {metric?.avg_hours_in_stage != null && (
                    <span style={{ fontSize: 9, color: '#A0AEC0' }}>{metric.avg_hours_in_stage < 24 ? `${metric.avg_hours_in_stage.toFixed(0)}h` : `${(metric.avg_hours_in_stage / 24).toFixed(1)}d`}</span>
                  )}
                ...
                {stageLeads.map(lead => (
                  <div key={lead.id} className={`kanban-card ${draggedLead === lead.id ? 'dragging' : ''}`}
                    ...
                    <button className="kanban-card-archive" title="Arquivar" onClick={e => handleArchive(e, lead.id)}
                      style={{ position: 'absolute', top: 6, right: 6, background: 'transparent', border: 'none', color: '#718096', cursor: 'pointer', padding: 4, borderRadius: 4, display: 'flex', opacity: 0.5 }}
