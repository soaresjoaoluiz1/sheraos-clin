              ...
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 8 }}>
                <div>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                    <Rocket size={14} style={{ color: '#5CB8B2' }} />
                    <h3 style={{ fontSize: 14, fontWeight: 600 }}>{l.title}</h3>
                  </div>
                  {l.identification && <div style={{ fontSize: 11, color: '#718096', marginTop: 2 }}>ID: {l.identification}</div>}
                </div>
              ...
              {l.messages.length > 0 && (
                <div style={{ marginTop: 8 }}>
                  {l.messages.slice(0, 2).map((m, i) => (
                    <div key={m.id} style={{ fontSize: 11, color: '#4A5568', padding: '4px 0', borderTop: i > 0 ? '1px solid var(--border-subtle)' : 'none' }}>
                      <strong style={{ color: '#5CB8B2' }}>P:</strong> {m.question.substring(0, 50)}{m.question.length > 50 ? '...' : ''}
                    </div>
                  ))}
                  {l.messages.length > 2 && <div style={{ fontSize: 10, color: '#A0AEC0', marginTop: 2 }}>+{l.messages.length - 2} mais</div>}
                </div>
              )}
            </div>
          ))}
