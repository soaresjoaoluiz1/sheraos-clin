...
const STATUS_MAP: Record<string, { label: string; color: string }> = {
  draft: { label: 'Rascunho', color: '#718096' },
      ...
      {loading ? <div className="loading-container"><div className="spinner" /></div> : (
        <div className="table-card">
          <table>
            ...
            <tbody>
              {broadcasts.map(b => {
                const st = STATUS_MAP[b.status] || { label: b.status, color: '#718096' }
                return (
                  <tr key={b.id}>
              ...
              {broadcasts.length === 0 && <tr><td colSpan={7} style={{ textAlign: 'center', padding: 40, color: '#A0AEC0' }}>Nenhum disparo criado</td></tr>}
            </tbody>
          </table>
        </div>
      )}
              ...
              <>
                ...
                {newTemplate && (
                  <div style={{ marginTop: 8, padding: 12, background: 'rgba(255,255,255,0.03)', borderRadius: 8, fontSize: 12 }}>
                    <div style={{ color: '#718096', marginBottom: 4 }}>Preview:</div>
            ...
            {step === 2 && (
              <>
                ...
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', margin: '8px 0' }}>
                  <span style={{ fontSize: 12, color: '#718096' }}>{selectedLeads.length} selecionados</span>
                ...
                <div style={{ maxHeight: 300, overflowY: 'auto', display: 'flex', flexDirection: 'column', gap: 4 }}>
                  {searchResults.map(l => (
                    <label key={l.id} style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '6px 8px', borderRadius: 6, background: selectedLeads.some(s => s.id === l.id) ? 'rgba(92,184,178,0.08)' : 'transparent', cursor: 'pointer', fontSize: 12 }}>
                      ...
                      <span style={{ color: '#718096' }}>{l.phone}</span>
                    </label>
                  ))}
                  {searchResults.length === 0 && leadSearch.length > 1 && <div style={{ padding: 20, textAlign: 'center', color: '#A0AEC0' }}>Nenhum lead encontrado</div>}
                  {leadSearch.length <= 1 && <div style={{ padding: 20, textAlign: 'center', color: '#A0AEC0' }}>Digite pra buscar leads...</div>}
                </div>
                <div className="modal-actions">
            ...
            {step === 3 && (
              <>
                <div className="card" style={{ marginBottom: 16 }}>
                  <div style={{ fontSize: 12, color: '#718096', marginBottom: 8 }}>Resumo</div>
                ...
                <div style={{ padding: 10, background: 'rgba(92,184,178,0.08)', borderRadius: 8, fontSize: 12, color: '#5CB8B2' }}>
