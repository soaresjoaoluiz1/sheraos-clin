        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
          ...
          <div>
            <h1 style={{ fontSize: 20, display: 'flex', alignItems: 'center', gap: 8 }}>
              {lead.name || 'Sem nome'}
              {lead.is_archived ? <span style={{ fontSize: 11, padding: '2px 8px', borderRadius: 12, background: '#71809620', color: '#718096', fontWeight: 500 }}>Arquivado</span> : null}
            </h1>
            {lead.phone && <div style={{ fontSize: 12, color: '#718096', display: 'flex', alignItems: 'center', gap: 4 }}><Phone size={10} /> {lead.phone}</div>}
          </div>
        </div>
        ...
        <div>
          ...
          <div className="card" style={{ marginBottom: 16 }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 8 }}>
              <div style={{ fontSize: 12, fontWeight: 600, color: '#718096', textTransform: 'uppercase' }}><TagIcon size={12} /> Tags</div>
                ...
                {showTagMenu && (
                  <div style={{ position: 'absolute', right: 0, top: '100%', marginTop: 4, background: 'var(--bg-card)', border: '1px solid var(--border-medium)', borderRadius: 8, padding: 6, zIndex: 50, minWidth: 220 }}>
                    {availableTags.length > 0 && (
                      <>
                        {availableTags.map(t => (
                          <button key={t.id} onClick={() => handleAddTag(t.id)} style={{ display: 'flex', alignItems: 'center', gap: 6, padding: '6px 10px', border: 'none', background: 'none', color: '#2D3748', fontSize: 12, cursor: 'pointer', borderRadius: 4, width: '100%', textAlign: 'left' }}>
            ...
            <div style={{ display: 'flex', gap: 4, flexWrap: 'wrap' }}>
              ...
              {(!lead.tags || lead.tags.length === 0) && <span style={{ fontSize: 11, color: '#A0AEC0' }}>Sem tags</span>}
            </div>
          </div>
          ...
          <div className="card" style={{ marginBottom: 16 }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 8 }}>
              <div style={{ fontSize: 12, fontWeight: 600, color: '#718096', textTransform: 'uppercase', display: 'flex', alignItems: 'center', gap: 4 }}><ListOrdered size={12} /> Cadencia</div>
              <div style={{ position: 'relative' }}>
                ...
                {showCadenceMenu && cadences.length > 0 && (
                  <div style={{ position: 'absolute', right: 0, top: '100%', marginTop: 4, background: 'var(--bg-card)', border: '1px solid var(--border-medium)', borderRadius: 8, padding: 6, zIndex: 50, minWidth: 180 }}>
                    {cadences.map(c => (
                      <button key={c.id} onClick={() => handleAssignCadence(c.id)} style={{ display: 'block', padding: '6px 10px', border: 'none', background: 'none', color: '#2D3748', fontSize: 12, cursor: 'pointer', borderRadius: 4, width: '100%', textAlign: 'left' }}>
            ...
            {leadCadence ? (
              <div>
                ...
                {leadCadence.status === 'completed' ? (
                  ...
                  <>
                    <div style={{ fontSize: 12, color: '#5CB8B2', display: 'flex', alignItems: 'center', gap: 4 }}>
                      Etapa {(leadCadence.attempt_position ?? 0) + 1}/{leadCadence.total_attempts}: {leadCadence.action_type?.toUpperCase()}
                    </div>
                    {leadCadence.attempt_description && <div style={{ fontSize: 11, color: '#4A5568', marginTop: 2 }}>{leadCadence.attempt_description}</div>}
                    {leadCadence.attempt_instructions && <div style={{ fontSize: 11, color: '#718096', marginTop: 2, fontStyle: 'italic' }}>{leadCadence.attempt_instructions}</div>}
              ...
              <span style={{ fontSize: 11, color: '#A0AEC0' }}>Sem cadencia atribuida</span>
            )}
          </div>
          ...
          {history.length > 0 && (
            <div className="card">
              <div style={{ fontSize: 12, fontWeight: 600, color: '#718096', textTransform: 'uppercase', marginBottom: 10 }}><GitBranch size={12} /> Historico</div>
              {history.slice(0, 10).map((h, i) => (
                <div key={h.id} style={{ display: 'flex', gap: 8, padding: '6px 0', borderBottom: i < Math.min(history.length, 10) - 1 ? '1px solid var(--border-subtle)' : 'none', fontSize: 11 }}>
                  <div style={{ width: 6, height: 6, borderRadius: '50%', background: '#5CB8B2', marginTop: 4, flexShrink: 0 }} />
                  <div>
                    <div style={{ color: '#2D3748' }}>{h.from_stage_name ? `${h.from_stage_name} → ${h.to_stage_name}` : `Entrada: ${h.to_stage_name}`}</div>
                    <div style={{ color: '#A0AEC0', fontSize: 10 }}>{h.trigger_type === 'manual' ? 'Manual' : h.trigger_type}{h.user_name ? ` por ${h.user_name}` : ''} · {new Date(h.created_at).toLocaleString('pt-BR')}</div>
                  </div>
                </div>
              ))}
            </div>
                ...
                {showReadyMsgs && readyMsgs.length > 0 && (
                  <div style={{ position: 'absolute', left: 0, bottom: '100%', marginBottom: 4, background: 'var(--bg-card)', border: '1px solid var(--border-medium)', borderRadius: 8, padding: 6, zIndex: 50, width: '100%', maxHeight: 200, overflowY: 'auto' }}>
                    {readyMsgs.filter(m => !m.stage_id || m.stage_id === lead.stage_id).map(m => (
                      <button key={m.id} onClick={() => handleSelectReadyMsg(m.content)} style={{ display: 'block', padding: '8px 10px', border: 'none', background: 'none', color: '#2D3748', fontSize: 12, cursor: 'pointer', borderRadius: 4, width: '100%', textAlign: 'left', borderBottom: '1px solid var(--border-subtle)' }}>
                        <div style={{ fontWeight: 600, marginBottom: 2 }}>{m.title}</div>
                        <div style={{ color: '#718096', fontSize: 11 }}>{m.content.substring(0, 80)}{m.content.length > 80 ? '...' : ''}</div>
                      </button>
                    ))}
                  </div>
                )}
          ...
          {activeTab === 'notes' && (
            <div className="card" style={{ minHeight: 400 }}>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
                {notes.map(n => (
                  <div key={n.id} style={{ padding: '10px 12px', background: 'rgba(92,184,178,0.05)', borderRadius: 8, border: '1px solid rgba(92,184,178,0.1)' }}>
                    <div style={{ fontSize: 13 }}>{n.content}</div>
                    <div style={{ fontSize: 10, color: '#A0AEC0', marginTop: 4 }}>{n.user_name} · {new Date(n.created_at).toLocaleString('pt-BR')}</div>
                  </div>
                ))}
                {notes.length === 0 && <div style={{ textAlign: 'center', color: '#A0AEC0', padding: 30 }}>Nenhuma nota ainda</div>}
              </div>
            </div>
          )}
              ...
              {qualifications.length > 0 ? (
                <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
                  {qualifications.map((q, i) => (
                    <div key={q.sequence_id} style={{ padding: '10px 12px', background: q.answer ? 'rgba(52,199,89,0.05)' : 'rgba(92,184,178,0.03)', borderRadius: 8, border: `1px solid ${q.answer ? 'rgba(52,199,89,0.15)' : 'var(--border-subtle)'}` }}>
                      <div style={{ display: 'flex', alignItems: 'center', gap: 6, marginBottom: 6 }}>
                        <div style={{ width: 20, height: 20, borderRadius: '50%', background: q.answer ? '#34C75920' : '#5CB8B220', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                          {q.answer ? <Check size={10} style={{ color: '#34C759' }} /> : <span style={{ fontSize: 10, fontWeight: 700, color: '#5CB8B2' }}>{i + 1}</span>}
                        </div>
                        <span style={{ fontSize: 13, fontWeight: 500 }}>{q.question}</span>
                      </div>
                      {q.answer ? (
                        <div style={{ marginLeft: 26 }}>
                          <div style={{ fontSize: 12, color: '#4A5568' }}>{q.answer}</div>
                          <div style={{ fontSize: 10, color: '#A0AEC0', marginTop: 2 }}>{q.answered_by_name} · {q.answered_at ? new Date(q.answered_at).toLocaleString('pt-BR') : ''}</div>
                        </div>
                      ) : (
                ...
                <div style={{ textAlign: 'center', color: '#A0AEC0', padding: 30 }}>Nenhuma pergunta de qualificacao configurada</div>
              )}
            </div>
          )}
            ...
            <div className="card" style={{ minHeight: 400 }}>
              {history.map((h, i) => (
                <div key={h.id} style={{ display: 'flex', gap: 10, padding: '8px 0', borderBottom: i < history.length - 1 ? '1px solid var(--border-subtle)' : 'none' }}>
                  <div style={{ width: 8, height: 8, borderRadius: '50%', background: '#5CB8B2', marginTop: 5, flexShrink: 0 }} />
                  <div>
                    <div style={{ fontSize: 13 }}>{h.from_stage_name ? `${h.from_stage_name} → ${h.to_stage_name}` : `Entrada: ${h.to_stage_name}`}</div>
                    <div style={{ fontSize: 11, color: '#A0AEC0' }}>{h.trigger_type}{h.user_name ? ` por ${h.user_name}` : ''}</div>
                    <div style={{ fontSize: 10, color: '#A0AEC0' }}>{new Date(h.created_at).toLocaleString('pt-BR')}</div>
                  </div>
                </div>
              ))}
              {history.length === 0 && <div style={{ textAlign: 'center', color: '#A0AEC0', padding: 30 }}>Sem historico</div>}
            </div>
          )}
        </div>
      </div>
