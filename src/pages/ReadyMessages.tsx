            ...
            <div key={m.id} className="card">
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 8 }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                  <MessageSquare size={14} style={{ color: '#5CB8B2' }} />
              ...
              <p style={{ fontSize: 12, color: '#4A5568', lineHeight: 1.5, maxHeight: 60, overflow: 'hidden' }}>{m.content}</p>
              <div style={{ display: 'flex', gap: 6, marginTop: 8, flexWrap: 'wrap' }}>
                {m.stage_name && (
                  <span className="stage-badge" style={{ background: `${m.stage_color || '#5CB8B2'}20`, color: m.stage_color || '#5CB8B2', fontSize: 10 }}>{m.stage_name}</span>
                )}
                {m.image_url && <span style={{ fontSize: 10, color: '#718096', display: 'flex', alignItems: 'center', gap: 2 }}><Image size={10} /> Imagem</span>}
                {m.video_url && <span style={{ fontSize: 10, color: '#718096', display: 'flex', alignItems: 'center', gap: 2 }}><Video size={10} /> Video</span>}
              </div>
            </div>
          ))}
