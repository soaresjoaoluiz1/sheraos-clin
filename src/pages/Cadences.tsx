              ...
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                <div style={{ cursor: 'pointer', flex: 1 }} onClick={() => setExpanded(expanded === c.id ? null : c.id)}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                    <ListOrdered size={16} style={{ color: '#5CB8B2' }} />
                    <h3 style={{ fontSize: 15, fontWeight: 600 }}>{c.name}</h3>
                    <span style={{ fontSize: 11, color: '#718096', background: 'rgba(155,150,176,0.1)', padding: '2px 8px', borderRadius: 10 }}>{c.attempts.length} etapas</span>
                    {expanded === c.id ? <ChevronUp size={14} style={{ color: '#A0AEC0' }} /> : <ChevronDown size={14} style={{ color: '#A0AEC0' }} />}
                  </div>
                  {c.description && <p style={{ fontSize: 12, color: '#718096', marginTop: 4 }}>{c.description}</p>}
                </div>
              ...
              {expanded === c.id && c.attempts.length > 0 && (
                <div style={{ marginTop: 12, display: 'flex', flexDirection: 'column', gap: 6 }}>
                  {c.attempts.map((a, i) => {
                    ...
                    return (
                      <div key={a.id} style={{ display: 'flex', gap: 10, alignItems: 'flex-start', padding: '8px 12px', background: 'rgba(92,184,178,0.03)', borderRadius: 8, border: '1px solid var(--border-subtle)' }}>
                        <div style={{ width: 24, height: 24, borderRadius: '50%', background: '#5CB8B220', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0, marginTop: 2 }}>
                          <span style={{ fontSize: 11, fontWeight: 700, color: '#5CB8B2' }}>{i + 1}</span>
                        </div>
                        <div style={{ flex: 1 }}>
                          <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
                            <Icon size={12} style={{ color: '#5CB8B2' }} />
                            <span style={{ fontSize: 12, fontWeight: 600, color: '#5CB8B2', textTransform: 'uppercase' }}>{at?.label || a.action_type}</span>
                          ...
                          {a.instructions && <div style={{ fontSize: 11, color: '#718096', marginTop: 2 }}>{a.instructions}</div>}
                        </div>
                      </div>
                    )
                  })}
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 8, marginTop: 12, maxHeight: 400, overflowY: 'auto' }}>
              {editAttempts.map((a, i) => (
                <div key={i} style={{ display: 'flex', gap: 6, alignItems: 'flex-start', padding: '8px', background: 'rgba(255,255,255,0.02)', borderRadius: 8, border: '1px solid var(--border-subtle)' }}>
                  <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', width: 24, color: '#718096', fontSize: 12, fontWeight: 700, paddingTop: 8 }}>{i + 1}</div>
                  <div style={{ flex: 1, display: 'flex', flexDirection: 'column', gap: 6 }}>
                    ...
                    <div style={{ display: 'flex', gap: 4, alignItems: 'center', marginBottom: 4 }}>
                      ...
                      <span style={{ fontSize: 10, color: '#A0AEC0', marginLeft: 4 }}>
                    ...
                    {a.schedule_mode === 'duration' ? (
                      <div style={{ display: 'flex', gap: 6, alignItems: 'center' }}>
                        ...
                        <span style={{ fontSize: 11, color: '#718096' }}>minutos depois</span>
                      </div>
                    ) : (
                      <div style={{ display: 'flex', gap: 6, alignItems: 'center' }}>
                        <div style={{ display: 'flex', alignItems: 'center', gap: 4 }}>
                          <span style={{ fontSize: 11, color: '#5CB8B2', fontWeight: 700 }}>D+</span>
                        ...
                        <span style={{ fontSize: 11, color: '#718096' }}>as</span>
