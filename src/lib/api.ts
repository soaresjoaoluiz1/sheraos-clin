const getToken = () => localStorage.getItem('sheraos_clin_token')
...
export async function apiFetch<T = any>(path: string, opts: RequestInit = {}): Promise<T> {
  ...
  if (res.status === 401) { localStorage.removeItem('sheraos_clin_token'); window.location.href = `${BASE}/login`; throw new Error('Unauthorized') }
