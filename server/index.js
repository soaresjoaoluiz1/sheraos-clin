...
// Strip /gestao-clin prefix so /clin/api/* maps to /api/*
app.use((req, res, next) => {
  if (req.url.startsWith('/clin/api/')) req.url = req.url.slice(12)
...
app.get('/api/events', async (req, res) => {
  ...
  try {
    ...
    user = jwtMod.default.verify(token, process.env.JWT_SECRET || 'sheraos-clin-secret-trocar')
...
if (fs.existsSync(distPath)) {
  ...
  // SPA fallback: any /clin/* route that doesn't match API → serve index.html
  app.get('/clin/*', (req, res) => {
    res.setHeader('Cache-Control', 'no-cache, no-store, must-revalidate')
    res.sendFile(resolve(distPath, 'index.html'))
  })
  console.log('[Sheraos Clin] Serving frontend from /clin/')
}

app.listen(PORT, () => {
  console.log(`[Sheraos Clin API] Running on http://localhost:${PORT}`)
  startScheduler()
})
